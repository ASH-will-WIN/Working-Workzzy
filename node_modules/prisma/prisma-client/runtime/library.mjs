...
${er(e)}`}var zm=L("prisma:client:engines:resolveEnginePath"),Zm=()=>new RegExp("runtime[\\\\/]library\\.m?js$");async function sl(e,r){let t={binary:process.env.PRISMA_QUERY_ENGINE_BINARY,library:process.env.PRISMA_QUERY_ENGINE_LIBRARY}[e]??r.prismaPath;if(t!==void 0)return t;let{enginePath:n,searchedLocations:i}=await Xm(e,r);if(zm("enginePath",n),n!==void 0&&e==="binary"&&yi(n),n!==void 0)return r.prismaPath=n;let o=await nr(),s=r.generator?.binaryTargets??[],a=s.some(d=>d.native),l=!s.some(d=>d.value===o),u=__filename.match(Zm())===null,c={searchedLocations:i,generatorBinaryTargets:s,generator:r.generator,runtimeBinaryTarget:o,queryEngineName:al(e,o),expectedLocation:jn.relative(process.cwd(),r.dirname),errorStack:new Error().stack},p;throw a&&l?p=il(c):l?p=tl(c):u?p=nl(c):p=ol(c),new T(p,r.clientVersion)}async function Xm(e,r){let t=await nr(),n=[],i=[r.dirname,jn.resolve(__dirname,".."),r.generator?.output?.value??__dirname,jn.resolve(__dirname,"../../../.prisma/client"),"/tmp/prisma-engines",r.cwd];__filename.includes("resolveEnginePath")&&i.push(ls());for(let o of i){let s=al(e,t),a=jn.join(o,s);if(n.push(o),Ym.existsSync(a))return{enginePath:a,searchedLocations:n}}return{enginePath:void 0,searchedLocations:n}}function al(e,r){return e==="library"?jt(r,"fs"):`query-engine-${r}${r==="windows"?".exe":""}`}var oo=ne(Si());function ll(e){return e?e.replace(/".*"/g,'"X"').replace(/[\s:\[]([+-]?([0-9]*[.])?[0-9]+)/g,r=>`${r[0]}5`):""}function ul(e){return e.split(`
